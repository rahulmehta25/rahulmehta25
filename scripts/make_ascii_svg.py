...
OUT = sys.argv[2] if len(sys.argv) > 2 else os.path.join(HERE, "..", "rahul-ascii.svg")
...
parts.append(
    ...
    f'text-anchor="middle">{HOST}: ~$ ./portrait.sh</text>'
)
...
parts.append(
    ...
    f'{HOST}:~$ whoami <tspan fill="{INK}">{WHOAMI}</tspan></text>'
)
# cursor sits after "Rahul Mehta" — approximate monospace width
cursor_x = PAD + 14 * 8.0 + len(WHOAMI) * 7.6
parts.append(
